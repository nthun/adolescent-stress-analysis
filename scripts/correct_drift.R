...
# Do the correction by using a 1 second rolling mean
ecg_ds <- 
    bvp %>% 
    group_by(id, task) %>% 
    mutate(rolmean = caTools::runmean(bvp, end_rate, align = "center"),
           corrected = bvp - rolmean)
...
# Save all data
pwalk(list(ecg_save$id, ecg_save$session, ecg_save$to_save), ~write_lines(x = ..3, path = str_glue("data/5_bvp_driftless/{..1}_{..2}_BVP.txt")))
